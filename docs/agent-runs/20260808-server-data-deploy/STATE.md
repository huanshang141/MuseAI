...
- 本地后端：`codex/data-driven-miniapp-framework`；最终会话修复提交 `033d3061eaade54b21176620bd8f533c9d7fddb4` 与文本日志修复提交 `fd96e17fd88ebd4d7426cee4bf8794a1c57c3871` 均已推送并完成生产验证，其前置配置提交为 `71d1ef205d6a7a68f0f98ed6e8aa2f9dd2f2ad72`。
- 小程序：唯一分支 `main`，最终提交 `99307705ace8ec66c0bcde80420067ac27b89ad5` 已推送到 `origin/main`，工作树干净。
- 生产后端：`/home/ubuntu/MuseAI` 以 detached HEAD 检出 `fd96e17fd88ebd4d7426cee4bf8794a1c57c3871`；`museai-backend` active，迁移为 `20260809_trusted_hall_chat_history (head)`，loopback 与公网 HTTPS readiness 均通过，旧 3000 无监听。
- 最新回退基线：发布记录 `/home/ubuntu/museai-config-backups/release_20260809T120456Z.env`；数据库备份 `/home/ubuntu/museai-backups/museai_20260809_200456_436886593.sql.gz`，SHA-256 `d7f8efcdaa5c5c0e9e6b9795cc2ddfad286f837f32ca803b3e2bfa653c8ead8c`；图片备份 `/home/ubuntu/museai-backups/exhibit-images_033d3061eaade54b21176620bd8f533c9d7fddb4-20260809T120456Z.tar.gz`，SHA-256 `77d673e07aa7c5b512affec9facf471829e44a13e6a1df9acfcb8a9dc6267b19`。图片持久目录为 `/home/ubuntu/museai-data/exhibit-images`。
...
2. 本地最终全量回归 `1589 passed, 23 skipped, 9 warnings`；最终会话/报告/契约定向回归 `549 passed`，日志与 TTS 组合 `90 passed`。Ruff、`uv lock --check`、`uv pip check`、Alembic 单一 head、模板/测试数据 dry-run 和 `git diff --check` 通过。
...
7. 186 条 grounding 独立矩阵和 8 条 turn-id 封闭探针全部通过；最终独立复核未发现 P0/P1。保留两个 P2：可信历史窗口外的极旧 `turn_id` 重试无法永久去重，以及非厅级对话会加载当前厅全部启用展品用于名称匹配。
8. 现场部署已完成真实数据库恢复演练、关闭旧 3000、容器 `unless-stopped` 和新迁移验证。Compose 与应用共用 `.env` 的三项配置已由提交 `71d1ef2` 严格接收并核对数据库 URL，且已部署验证。
9. 最终提交 `033d3061` 将页面选中展品与本轮对象分离，持久化私有对象范围/澄清元数据，并覆盖明确 B、A/B 比较、身份核对、模糊代词、复数、序数和双示指。公网探针验证报告仅计 1 个有效问题、恢复历史不泄漏私有字段；SSE TTS 返回 4,392,960 字节有效 PCM，独立 TTS 返回 122,924 字节有效 WAV。
10. 生产 TTS 音频正常但含字典的调试日志曾触发 Loguru 文本文件 handler 二次格式化，产生 `KeyError: "'name'"`。提交 `fd96e17` 改为返回固定 Loguru 模板并加入 `catch=False` 回归；部署后真实 TTS 返回 92,204 字节有效 WAV，重启后 `Logging error`、`KeyError`、`Traceback` 计数均为 0。

## 待完成

1. 取得现有 `test@test.com` 管理员口令，或经用户明确授权重置后，完成一次生产图片上传、公开读取、删除和空状态恢复烟测；本轮不得擅自更改口令。
2. 接入真实馆方数据前评估单厅展品规模；若显著超过当前 46 条测试数据规模，将聊天名称匹配改为数据库筛选或缓存，并考虑为可信历史建立独立持久化 `turn_id` 去重账本。

## 2026-08-09 最终会话检查点（已部署）
...
- 验证：最终全量 `1589 passed, 23 skipped, 9 warnings`，无失败；最终三文件定向组合 `549 passed`，日志与 TTS 组合 `90 passed`，grounding 精确矩阵 `186/186`，turn-id 封闭探针 `8/8`。scoped Ruff、`uv lock --check`、`uv pip check`、两套 CSV dry-run、Alembic 单一 head 与直接降到 `20260809_exhibit_images` 的离线 SQL、备份/Compose 门禁及 `git diff --check` 均通过。
