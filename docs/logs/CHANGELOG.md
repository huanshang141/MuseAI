...
- 发布文档按用户真机验收结果收口：中英文 README 将阶段更新为体验版与真实数据上线准备，记录当前候选已通过关闭合法域名豁免的真机回归，体验版上传和成员分发仍待执行。
- `docs/miniapp-content-maintenance.md` 补齐原缺失的第 8 节，按前端、后端、迁移、真实数据、图片和受控配置分类发布；新增正式上线前门禁、上线当天串行顺序、发布后 30 分钟/24 小时检查、日常维护节奏和停止发布/回退触发条件。
- 权威命令边界保持不变：第 7 节仍是精确 SHA、备份、依赖、迁移和 systemd 发布唯一入口，第 9 节仍是数据库/图片/代码回退入口；`deploy/DEPLOYMENT_NOTES.md` 只增加导航，不复制第二套发布命令。
- 提交前实时核对本项目证书职责：2026-08-11 MuseAI 根域名与 `www` 仍解析到本服务器，公网响应来自 Nginx，API 手工证书与 MuseAI 官网 Certbot 证书均被生效配置引用，必须分别维护；同时删除此前误套用外部项目迁移状态的错误口径。
- `docs/server-storage-layout.md` 同步上述证书与 DNS 边界，并加入第 7 节部署后纯文档同步白名单；该白名单仍只允许明确列出的 Markdown，任何运行文件差异都必须作为新发布处理。
- 验证通过：Markdown 相对链接和 UTF-8 文本检查、16 个 Bash 文档块与 2 个部署脚本语法、`uv lock --check`、纯文档差异门禁和 `git diff --check` 均通过；本轮未修改运行代码、依赖或迁移，因此未重复执行后端全量 pytest。
